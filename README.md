...
You can create a smaller test dataset by passing parameters:
...
Create a database, run the DDL, then import the CSVs.

```bash
createdb asset_management_growth
psql -d asset_management_growth -f sql/create_tables.sql
